...
public class ParentScreenTimeService {
    ...
    public ResponseEntity<Map<String, Object>> getScreenTimeGraph(String user_id, String targetDate) throws Exception {
        ...
        if(!isCollectLinkedID(user_id, child_id))
            throw new CustomException("child_id 중 올치 않은 계정이 있습니다.", null, HttpStatus.BAD_REQUEST, null);
    ...
    public ResponseEntity<Map<String, Object>> getScreenTime(String user_id, String targetDate) throws Exception {
        ...
        if(!isCollectLinkedID(user_id, child_id))
            throw new CustomException("child_id: "+ child_id + "는 연동된 계정이 아닙니다.", null, HttpStatus.BAD_REQUEST, null);
        ...
        try{
            if(graphData != null){
                if(!Objects.equals(graphData.getUser_id(), childId)){
                    throw new CustomException("해당 그래프 데이터에 대한 접근 권한이 없습니다.", null, HttpStatus.NOT_ACCEPTABLE, null);
                }
        ...
        try{
            if(screenTime != null){
                if(!Objects.equals(screenTime.getUser_id(), childId)){
                    throw new CustomException("해당 스크린타임 시간 데이터에 대한 접근 권한이 없습니다.", null, HttpStatus.NOT_ACCEPTABLE, null);
                }
    ...
    private List<String> getTargetChildID(String parent_id){;
        ...
        if(child_id_list == null){
            throw new CustomException("해당 부모와 연동된 아이가 존재하지않습니다.", null, HttpStatus.NOT_FOUND, null);
        }
